/**********************************************************************************
    A Growtopia Private Server made with pure ENet,
	Made for reliability in private server making.
	Including All Bases and Items.Dat Version 14 Support
	With A Config.json to edit the port and cache used in game
	
	Copyright Claimed (C) by Anonymous Factor
**********************************************************************************/
...
#define Property_Foreground 8192
			...
			{
				def.blockType = BlockTypes::VIPER;
			}
	...
	*/
	...
	if (ifs.is_open()) {
		...
		try {
			...
			string config_contents = "{ \"port\": 17091, \"cdn\": \"0098/24450/cache/\" }";
...
#endif
{
	cout << "A Growtopia Private Server made with pure ENet" << endl;
		
	cout << "By The Anonymous Factors" << endl;
	loadConfig();
	...
	while (enet_host_service(server, &event, 1000) > 0)
	{
		...
		case ENET_EVENT_TYPE_CONNECT:
		{
			...
			if (count > 3)
			{
				packet::consolemessage(peer, "`4Too many accounts are currently online with that IP, try again later.``");
		...
		case ENET_EVENT_TYPE_RECEIVE:
		{
			...
			case 2:
			{
				...
				else if (cch.find("action|growid") == 0) {
